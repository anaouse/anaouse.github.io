...
单调区间就是求导，二阶导小于0，凸，大于0就是凹，拐点就是凸和凹的交界
...
由于积分区域是圆形，并且被积函数包含 $x^2+y^2$ 项，通常考虑使用极坐标。然而，该圆的圆心并非原点。我们可以使用平移后的极坐标变换（也称为广义极坐标变换）。
令：
$x - 2 = r \cos\theta$
$y - 1 = r \sin\theta$
即：
...
r
r
，所以面积元素 $d\sigma = dx dy = r dr d\theta$。
积分区域 
D
D
 在新的 $r, \theta$ 坐标下变为：
$(r \cos\theta)^2 + (r \sin\theta)^2 \le 3$
$r^2 (\cos^2\theta + \sin^2\theta) \le 3$
$r^2 \le 3$
所以，
r
r
 的范围是 $0 \le r \le \sqrt{3}$，
θ
θ
 的范围是 $0 \le \theta \le 2\pi$。

被积函数是 $x^2 + y^2$。将 $x = 2 + r \cos\theta$ 和 $y = 1 + r \sin\theta$ 代入：
$x^2 + y^2 = (2 + r \cos\theta)^2 + (1 + r \sin\theta)^2$
$= (4 + 4r \cos\theta + r^2 \cos^2\theta) + (1 + 2r \sin\theta + r^2 \sin^2\theta)$
$= 4 + 4r \cos\theta + r^2 \cos^2\theta + 1 + 2r \sin\theta + r^2 \sin^2\theta$
$= 5 + 4r \cos\theta + 2r \sin\theta + r^2 (\cos^2\theta + \sin^2\theta)$
$= 5 + 4r \cos\theta + 2r \sin\theta + r^2$ (因为 $\cos^2\theta + \sin^2\theta = 1$)
知识点：三角恒等式 $\cos^2\theta + \sin^2\theta = 1$。

原积分为 $\iint_D (x^2 + y^2) d\sigma$。转换后变为：
$I = \int_0^{2\pi} \int_0^{\sqrt{3}} (5 + 4r \cos\theta + 2r \sin\theta + r^2) r dr d\theta$
$I = \int_0^{2\pi} \int_0^{\sqrt{3}} (5r + 4r^2 \cos\theta + 2r^2 \sin\theta + r^3) dr d\theta$

$\int_0^{\sqrt{3}} (5r + 4r^2 \cos\theta + 2r^2 \sin\theta + r^3) dr$
$= [\frac{5}{2}r^2 + \frac{4}{3}r^3 \cos\theta + \frac{2}{3}r^3 \sin\theta + \frac{1}{4}r^4]_0^{\sqrt{3}}$
$= (\frac{5}{2}(\sqrt{3})^2 + \frac{4}{3}(\sqrt{3})^3 \cos\theta + \frac{2}{3}(\sqrt{3})^3 \sin\theta + \frac{1}{4}(\sqrt{3})^4) - 0$
$= \frac{5}{2}(3) + \frac{4}{3}(3\sqrt{3}) \cos\theta + \frac{2}{3}(3\sqrt{3}) \sin\theta + \frac{1}{4}(9)$
$= \frac{15}{2} + 4\sqrt{3} \cos\theta + 2\sqrt{3} \sin\theta + \frac{9}{4}$
$= \frac{30}{4} + \frac{9}{4} + 4\sqrt{3} \cos\theta + 2\sqrt{3} \sin\theta$
$= \frac{39}{4} + 4\sqrt{3} \cos\theta + 2\sqrt{3} \sin\theta$
知识点：定积分的基本计算，幂函数的积分 $\int x^n dx = \frac{x^{n+1}}{n+1} + C$。


$I = \int_0^{2\pi} (\frac{39}{4} + 4\sqrt{3} \cos\theta + 2\sqrt{3} \sin\theta) d\theta$
$= [\frac{39}{4}\theta + 4\sqrt{3} \sin\theta - 2\sqrt{3} \cos\theta]_0^{2\pi}$
$= (\frac{39}{4}(2\pi) + 4\sqrt{3} \sin(2\pi) - 2\sqrt{3} \cos(2\pi)) - (\frac{39}{4}(0) + 4\sqrt{3} \sin(0) - 2\sqrt{3} \cos(0))$
$= (\frac{39\pi}{2} + 4\sqrt{3}(0) - 2\sqrt{3}(1)) - (0 + 4\sqrt{3}(0) - 2\sqrt{3}(1))$
$= (\frac{39\pi}{2} - 2\sqrt{3}) - (-2\sqrt{3})$
$= \frac{39\pi}{2} - 2\sqrt{3} + 2\sqrt{3}$
$= \frac{39\pi}{2}$
知识点：三角函数的积分 $\int \cos\theta d\theta = \sin\theta + C$，$\int \sin\theta d\theta = -\cos\theta + C$。
知识点：三角函数在一个周期（如 $0$ 到 $2\pi$）上的性质：$\sin(2\pi) = \sin(0) = 0$，$\cos(2\pi) = \cos(0) = 1$。
一个有用的结论：对于整数 
n
≥
1
n≥1
，$\int_0^{2\pi} \cos(n\theta) d\theta = 0$ 和 $\int_0^{2\pi} \sin(n\theta) d\theta = 0$。这里我们直接计算了，但注意到 $4\sqrt{3} \cos\theta$ 和 $2\sqrt{3} \sin\theta$ 这两项在 $0$到$2\pi$上的积分都为零，可以简化心算。
